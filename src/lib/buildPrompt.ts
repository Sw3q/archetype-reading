...
export function buildPrompt(
  ...
  const cardLines = byCloseness
    .map((s, i) => {
      const a = s.archetype
      const reclaimed = reclaimedIds.has(a.id)
  ...
  return `You are a Jungian depth psychoanalyst who also knows Caroline Myss's "Sacred Contracts" intimately — her Gallery of Archetypes, the survival archetypes, and her light/shadow framework. I have just completed an archetype roundtable reading drawn from Myss's gallery of 91 archetypes, and I want to explore it with you in a depth-psychology session.
...
1. Briefly reflect the table back to me in your own words — what stands out structurally: what sits nearest, what is held at the rim, which alliances look load-bearing, and what the shadow-reclaimed cards change about the picture.
